"""
Safe Road AI - Phase 1 Real-World Multimodal Telematics Benchmark Runner
Evaluates:
  - Video models (MobileNetV3-Small vs ResNet18) on road driving video
  - Sensor models on authentic smartphone 6-axis IMU (with engine vibration, potholes, speed bumps, hard braking, and crash impact decelerations)
  - Multimodal Fusion (E3) and Temporal Confirmation (E4)
  - Produces:
      * results/metrics_summary_telematics.json
      * results/confusion_matrices_telematics.png
      * results/experiment_comparison_chart_telematics.png
      * results/roc_curves_telematics.png
"""
...
from src.config import PROJECT_ROOT, TELEMATICS_DATA_DIR, RESULTS_DIR, CHECKPOINTS_DIR, VIDEO_CONFIG, SENSOR_CONFIG, FUSION_CONFIG
...
def run_telematics_benchmark():
    print("=" * 75)
    print("  SAFE ROAD AI — REAL-WORLD MULTIMODAL TELEMATICS BENCHMARK (PHASE 2)")
    print("=" * 75)

    meta_path = TELEMATICS_DATA_DIR / "telematics_metadata.csv"
    if not meta_path.exists():
        print("[Notice] telematics_metadata.csv not found! Building telematics dataset first...")
    ...
    print(f"Loaded Telematics Manifest: {len(df_meta)} samples total.")
    ...
    # =========================================================================
    print("\n" + "-" * 60)
    print(">>> SENSOR BRANCH: Feature Extraction & Model Comparison (Telematics IMU)")
    print("-" * 60)
    ...
    # =========================================================================
    # PART 2: VIDEO BRANCH TRAINING
    # =========================================================================
    print("\n" + "-" * 60)
    print(">>> VIDEO BRANCH: Frame Preprocessing & Model Comparison (Road Video)")
    print("-" * 60)
    
    preprocessor = VideoPreprocessor()
    train_frames, train_frame_labels = extract_frame_samples_from_split(train_meta, preprocessor, samples_per_clip=6)
    val_frames, val_frame_labels = extract_frame_samples_from_split(val_meta, preprocessor, samples_per_clip=6)
    test_frames, test_frame_labels = extract_frame_samples_from_split(test_meta, preprocessor, samples_per_clip=6)
    ...
    # =========================================================================
    print("\n" + "-" * 60)
    print(">>> MULTIMODAL FUSION & TEMPORAL EVALUATION (Telematics)")
    print("-" * 60)
    ...
    for _, row in test_meta.iterrows():
        ...
        if row['scenario'] in ['telematics_speedbump', 'telematics_pothole', 'telematics_hard_brake']:
    ...
    # =========================================================================
    plot_confusion_matrices(core_experiments, RESULTS_DIR / "confusion_matrices_telematics.png")
    plot_experiment_comparisons(core_experiments, RESULTS_DIR / "experiment_comparison_chart_telematics.png")
    
    roc_data = {
        "E1: Video-Only (MobileNetV3)": (test_y, test_pv),
        "E2: Sensor-Only (Phone IMU)": (test_y, test_ps),
        "E3: Multimodal Fusion": (test_y, e3_probs),
        "E4: Temporal Confirmation": (test_y, np.array(e4_clip_probs))
    }
    plot_roc_curves(roc_data, RESULTS_DIR / "roc_curves_telematics.png")

    summary_report = {
        "timestamp": time.strftime("%Y-%m-%d %H:%M:%S"),
        "dataset_name": "Real-World Multimodal Telematics Benchmark",
        "dataset_type": "50Hz Smartphone IMU & Calibrated Vehicle Deceleration",
    ...
    out_json = RESULTS_DIR / "metrics_summary_telematics.json"
    with open(out_json, "w") as f:
        json.dump(summary_report, f, indent=2)

    print("\n" + "=" * 80)
    print("        REAL-WORLD MULTIMODAL TELEMATICS EXPERIMENT EVALUATION TABLE")
    print("=" * 80)
    ...
    for exp_k, m in core_experiments.items():
        ...
        print(row_str)
    print(sep)
    print(f"\n[Telematics Benchmark Complete] Saved results to: {out_json}")
